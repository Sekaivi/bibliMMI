        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h2 class="text-lg font-medium text-gray-900">Nouvel exemplaire pour {{$ouvrage->titre}}</h2>
                    @if(session('status'))
                    ...
                    <form action="{{ route('ouvrages.exemplaires.store' , $ouvrage->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div>
                            ...
                            <select {{$required}} class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full"
                                ...
                                <option value="{{ $option->id }}">{{ $option->nom }} {{ $option->prenom }}</option>
                                @else
                            ...
                            <input value="1" type="checkbox" id="{{ $cle }}" name="{{ $cle }}" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                            @else
                            <input type="{{ $type }}" id="{{ $cle }}" name="{{ $cle }}" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full" value="">
                            @endif
